...
# ---------------------------------------------------------------------------  
  ...
  def computeStdDevInsideMask(self, subtractionOutputVolumeNode, maskVolumeNode):
    '''
    :param subtractionOutputVolumeNode: the standard deviation of this volume is computed
    :param maskVolumeNode: std. dev. is computed using the voxels in which mask=1 
    '''
  ...
  def runGenerateMask(self, basalVolumeNode, ictalVolumeNode, threshold, zmax):
    '''
    This method executes the method generateMask() in a new thread. Also shows a message while generateMask() is running  
    :param basalVolumeNode: Parameter used by generateMask()
    :param ictalVolumeNode: Parameter used by generateMask()
    :param threshold: Parameter used by generateMask()
    :param zmax: Parameter used by generateMask()
    '''
  ...
  def generateMask(self, basalVolumeNode, ictalVolumeNode, threshold, zmax):   
    '''
    A mask is created using the following procedure:
      1- primary basal mask (brain region) is created with voxels whose intensities are bigger than threshold * maxBasalValue 
      2- a primary ictal mask (brain region) is created with voxels whose intensities are bigger than threshold * maxIctalValue 
      3- Mean and std deviation of pixels inside the mask are computed
      4- A basal map and an ictal map are created with the z-scores of both studies that are below the parameter zmax
      5- The intersection region is computed as the region where both maps are positive.
      6- Original studies are normalized using the basal_normalization_factor and ictal_normalization_factor. 
         basal_normalization_factor = basal / mean(basal[intersection_region])
         ictal_normalization_factor = ictal / mean(ictal[intersection_region])
      7- basalMask = normalizedBasalArray > 0.4 * max_norm_basal
         ictalMask = normalizedIctalArray > 0.4 * max_norm_ictal 
         mask = basalMask * ictalMask  
    :param basalVolumeNode:
    :param ictalVolumeNode:
    :param threshold:
    :param zmax:
    '''
    self.userMessage = "Generating mask. Please wait..."     
  ...
  def runFillMaskTest(self):
   ...
   #--------------------------------------------------------------------------------       
  def fillMask(self, mascara, metodo, radio):
    # function [mascara_nueva,vecindad]=fillMask(mascara,metodo,radio)
...
#                mascara_nueva=M<1;
  ...
  def subtractImages(self):  
    ...
    if outputVolumeNode is None:
      ...
      outputVolumeNode.SetName(self.ICTAL_BASAL_SUBTRACTION)
      '''
